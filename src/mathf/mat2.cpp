...
namespace Odin {
	...
	template <typename Type> inline Mat2<Type>& Mat2<Type>::operator -(void) {
		Type a11 = m_mat[0],
		     a12 = m_mat[1],
		     a21 = m_mat[2],
		     a22 = m_mat[3],
	...
	template <typename Type> inline Mat2<Type>& Mat2<Type>::operator *=(Type s) {
		m_mat[0] *= s;
		m_mat[1] *= s;
		m_mat[2] *= s;
		m_mat[3] *= s;
	...
	template <typename Type> inline std::string Mat2<Type>::ToString(int32 p) {
		...
		return (
		           "Mat2[\n"
		           "  " + Odin::ToString(m_mat[0], p) + ", " + Odin::ToString(m_mat[2], p) + ",\n" +
		           "  " + Odin::ToString(m_mat[1], p) + ", " + Odin::ToString(m_mat[3], p) + "\n" +
		           "]"
		       );
	}
}
