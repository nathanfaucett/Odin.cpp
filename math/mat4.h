		...
		public:
			...
			inline Mat4& Decompose(Vec3& position, Vec3& scale, Quat& rotation) {
				float m11 = mat[0],
					...
					sx = Mathf.V3Length(m11, m21, m31),
					sy = Mathf.V3Length(m12, m22, m32),
					sz = Mathf.V3Length(m13, m23, m33),
			...
			inline Mat4& Frustum(const float left, const float right, const float top, const float bottom, const float near, const float far) {
				...
				mat[0] = x; mat[4] = 0; mat[8] = a; mat[12] = 0;
				mat[1] = 0; mat[5] = y; mat[9] = b; mat[13] = 0;
				mat[2] = 0; mat[6] = 0; mat[10] = c; mat[14] = d;
			...
			inline Mat4& Perspective(const float fov, const float aspect, const float near, const float far) {
				float ymax = near * std::tan(fov * 0.5),
					ymin = -ymax,
					xmin = ymin * aspect,
					xmax = ymax * aspect;
